...
# # Input
#"/home/tuba/herpov/netTCR_public_data/data/donor4/all_contig_annotations.csv" #
CONTIG_FILES = snakemake.params.contig #"/home/tuba/herpov/tcr-pmhc-sc-project/data/exp9_TCR/processed/cellranger_out/TCR_VDJ/outs/all_contig_annotations.csv"
CLONOTYPES = snakemake.params.clonot #"/Volumes/tuba/herpov/tcr-pmhc-sc-project/data/" + EXP + "_TCR/processed/cellranger_out/TCR_VDJ/outs/consensus_annotations.csv"

for filename in CONTIG_FILES:
    if 'tot' in filename:
        INPUT = filename
...
# # Output
#"/home/tuba/herpov/netTCR_public_data/data/donor4/tcr.clean.augmented.csv" #
#"/home/tuba/herpov/netTCR_public_data/data/donor4/tcr.clean.augmented.txt" #
OUTPUT = snakemake.output[0] #"/home/tuba/herpov/tcr-pmhc-sc-project/data/exp9_TCR/augmented/tcr.clean.augmented.old.csv" #
report = snakemake.output[1] #"/home/tuba/herpov/tcr-pmhc-sc-project/data/exp9_TCR/augmented/tcr.clean.augmented.old.txt" #
...
try:
    ...
    neg_gems = set(neg_df.barcode) - pos_gems # why do we see intersection?
except:
    neg_gems = set()

print('pos_gems', len(pos_gems))
print('neg_gems', len(neg_gems))
...
################################
print('name non-consensus clonotypes')
...
def annotate_clonotype(row, max_ct_num=max_ct_num):
    if row.clone_repl == row.clone_repl:
        ...
        if len(spl) == 1:
            print('nan cts', row.gem, row.clonotype, row.imp_clone, row.new_clone, row.clone_repl)
            return 0 #np.nan
        return int(row.clonotype.split('clonotype')[1])
    else:
        print('else', row.clonotype, row.imp_clone, row.new_clone, row.clone_repl)
...
print(len(flt_df), print(flt_df.ct.nunique()))
print(len(flt_df.ct.dropna()), print(flt_df.ct.dropna().nunique()))
...
print('TRA gems', tra_df.gem.unique().shape[0])
print('TRB gems', trb_df.gem.unique().shape[0])
print(tcr_df.shape[0], tcr_df.dropna(subset=['clonotype']).shape[0])
print(tcr_df.gem.unique().shape[0], tcr_df.dropna(subset=['clonotype']).gem.unique().shape[0])
#print(tcr_df.loc[tcr_df.duplicated(subset='gem',keep=False), ['gem','clonotype','ct', 'chain']].sort_values(by='gem'))
...
tcr_df.loc[tcr_df.gem.isin(pos_gems), 'label'] = 1 #tcr_df.gem.isin(pos_gems).astype(int)
tcr_df.loc[tcr_df.gem.isin(neg_gems), 'label'] = 0

...
plot_type = snakemake.params.plot_type #regex.group(1)
data_type = snakemake.params.data_type #regex.group(2)
thrs_type = snakemake.params.thrs_type #regex.group(3)

opt_thr = pd.read_csv(THRESHOLD, index_col=0, header=None)
opt_thr.dropna(inplace=True, inplace=True) #.fillna(0, inplace=True) # Hacks

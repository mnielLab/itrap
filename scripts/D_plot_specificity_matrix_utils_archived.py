...
def filter_low_level(credible_df, read_threshold, tcr_threshold, exclude_clonotype_singlets, exclude_specificity_singlets, clonotype_fmt, filtration):
    if (filtration == 'no_filtration'):
        ...
        flt_df = credible_df[credible_df.apply(lambda row: f(row), axis=1).values].copy()
    else:
        flt_df = credible_df[(credible_df.umi_count_mhc >= read_threshold) &
                             (credible_df.umi_count_TRA >= tcr_threshold) &
                             (credible_df.umi_count_TRB >= tcr_threshold)].copy()
...
def peptide_per_clonotype_by_gem_size(credible_df, clonotype_fmt="ct", filtration="no_filtration", bc_threshold=1, tcr_threshold=1, exclude_clonotype_singlets=False, exclude_specificity_singlets=False, show=True, save_tuba=False, save_sund=False, save_report=False):
    ...
    flt_df = filter_low_level(credible_df, bc_threshold, tcr_threshold, exclude_clonotype_singlets, exclude_specificity_singlets, clonotype_fmt, filtration)
    assert len(flt_df) > 0, print(clonotype_fmt, filtration, bc_threshold, tcr_threshold, exclude_clonotype_singlets, exclude_specificity_singlets)
    
    flt_df = calc_binding_concordance(flt_df, clonotype_fmt)
    ...
    if show:
        ...
        plt.show()
        print("OBS! Figures are not saved!")
        np.savetxt(show + 'unique_gems.lst', list(unique_gems), fmt='%s')

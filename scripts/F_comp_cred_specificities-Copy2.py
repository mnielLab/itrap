...
class Evaluate_Clonotype():
    ...
    def plot_advanced_figure(self, figname):
        ...
        h,l = get_legend_n_handle(l, h, key='umi')
        ...
        plot = ax3.scatter([np.nan]*len(order), order[::-1], c=[np.nan]*len(order), cmap='viridis_r', vmin=0, vmax=1) #[],[]
...
for ct in selected_clonotypes.index:
    ...
    #except:
    #    print('did not print ct', ct)
    #    plt.close()
    #    plt.clear()

df['pep_match'] = df.ct_pep == df.peptide_HLA
dct = dict.fromkeys(Evaluate_Clonotype.value_bin, True)
df['valid_ct'] = df.ct.map(dct)
